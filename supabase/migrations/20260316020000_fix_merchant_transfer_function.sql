-- Fix merchant transfer function - drop and recreate to clear any cached references
DROP FUNCTION IF EXISTS public.transfer_my_personal_wallet_to_merchant(NUMERIC, TEXT, TEXT);

-- Recreate function with correct table references
...
BEGIN
  ...
  -- Calculate current merchant available balance using same logic as get_my_merchant_balance_overview
  SELECT
