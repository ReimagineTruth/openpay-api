-- Complete fix for payment link workflow
-- This script adds missing columns and recreates the function with correct signature

-- First, add missing columns to merchant_payment_links table
DO $$
BEGIN
    -- Add total_amount column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'merchant_payment_links' 
        AND column_name = 'total_amount'
        AND table_schema = 'public'
    ) THEN
        ALTER TABLE public.merchant_payment_links 
        ADD COLUMN total_amount NUMERIC(12,2);
        RAISE NOTICE 'Added total_amount column to merchant_payment_links';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.merchant_payment_links 
        ADD COLUMN api_key_id UUID REFERENCES public.merchant_api_keys(id);
        RAISE NOTICE 'Added api_key_id column to merchant_payment_links';
    END IF;
END $$;
...
BEGIN
  ...
  -- Authenticate merchant via API key (support both MD5 and SHA256 hashes)
...
DECLARE
    func_count INTEGER;
    col_total_exists BOOLEAN;
    col_api_key_exists BOOLEAN;
BEGIN
    ...
    SELECT EXISTS (
        ...
        AND column_name = 'api_key_id'
        AND table_schema = 'public'
    ) INTO col_api_key_exists;
    
    IF func_count > 0 AND col_total_exists AND col_api_key_exists THEN
        RAISE NOTICE 'SUCCESS: Payment link function and table structure fixed';
        RAISE NOTICE '- Function created with correct 21-parameter signature';
        RAISE NOTICE '- total_amount column added to merchant_payment_links';
        RAISE NOTICE '- api_key_id column added to merchant_payment_links';
        RAISE NOTICE '- Schema cache refresh notification sent';
    ELSE
        RAISE EXCEPTION 'ERROR: Fix was not applied completely';
    END IF;
END $$;
