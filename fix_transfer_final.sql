-- Final fix for transfer function schema cache issue
-- This script ensures the transfer function is properly available and cached

-- Step 1: Completely drop the function to clear all cached references
DROP FUNCTION IF EXISTS public.transfer_my_personal_wallet_to_merchant(NUMERIC, TEXT, TEXT) CASCADE;

-- Step 2: Recreate the function with explicit schema declaration
...
-- Step 5: Add comment for debugging
COMMENT ON FUNCTION public.transfer_my_personal_wallet_to_merchant(NUMERIC, TEXT, TEXT) IS 'Transfer funds from personal wallet to merchant wallet. Final schema cache fix applied.';

-- Step 6: Verify function is properly registered
SELECT 
